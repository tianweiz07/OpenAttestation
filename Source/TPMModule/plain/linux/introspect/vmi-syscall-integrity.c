...
int introspect_syscall_check(char *uuid) {
    ...
    strcat(file_address, "/syscall_index");
    ...
    while(fgets(_line, sizeof(_line), _file) != NULL){
        ...
        strcpy(sys_index[num_sys-1], "sys_");
        strcat(sys_index[num_sys-1], _name);
    }
    fclose(_file);
    ...
    for (i=start_index; i<end_index; i++) {
        ...
        if (sys_call_addr < kernel_start || sys_call_addr > kernel_end) {
            /**
             * Sys call has been changed. Need to be fixed
             */
//            printf("sys_call %s address changed to 0x%" PRIx64 "\n", sys_index[i], sys_call_addr);
            sys_call_addr = vmi_translate_ksym2v(vmi, sys_index[i]);
            vmi_write_addr_va(vmi, sys_call_table_addr+i*8, 0, &sys_call_addr);
            count_syscall ++;
        }
    }
